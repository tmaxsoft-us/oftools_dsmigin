...
class ListcatJob(Job):
    ...
    def _analyze(self, record):
        ...
        Log().logger.debug('[listcat] Assessing dataset eligibility: ' + record[
            Col.DSN.value])
        ...
        if record[Col.IGNORE.value] == 'Y':
            ...
            Log().logger.info(skip_message + 'LISTCAT set to "N"')
            rc = 1
        ...
        if rc == 0:
            Log().logger.debug('[listcat] Dataset eligible: ' + record[
            Col.DSN.value] + ': Proceeding' )
    ...
    def _get_dataset_info(self, record):
        """Executes the FTP command on Mainframe to retrieve dataset info.
            ...
            Returns:
                integer -- Return code of the method."""
        Log().logger.debug('[listcat] Retrieving dataset info from Mainframe: ' +
        ...
        if rc == 0:
            ...
            if len(fields) > 7:
                ...
                record[Col.BLKSIZE.value] = fields[7]
                record[Col.DSORG.value] = fields[8]
